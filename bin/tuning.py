...
from load import load_data, data_transform_2, save_cache, load_cache
...
def tuning(methodology, binsize, combinations_size):
    drop_fields = []
    ...
    filepath_cache_ii = "{}/../input/transform2=True_testing=-1_type=2_binsize={}_combination={}.cache.pkl".format(BASEPATH, binsize, combinations_size)
    filepath_tuning = "{}/../etc/parameter_tuning/{}_binsize={}_combination={}.pkl".format(BASEPATH, methodology, binsize, combinations_size)

    algorithm, train_x = None, None
    if os.path.exists(filepath_tuning):
        algorithm = load_cache(filepath_tuning)

        log("{} data records with {} features".format(len(algorithm.train_x), len(algorithm.train_x.columns)))
    else:
