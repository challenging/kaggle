...
def worker(working_queue, folder_submission):
    while True:
        ...
        best_hotels_search_dest, best_hotels_od_ulc, best_hotels_country, best_hotels_weekday, popular_hotel_cluster = prepare_arrays_match(filepath_training)
        with open(filepath_submission, "wb") as OUTPUT:
            for user_id, filled in gen_submission(filepath_testing, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_country, best_hotels_weekday, popular_hotel_cluster):
...
def main(f_pattern, n_threads=4):
    ...
    folder_submission = folder.replace("test", "submission_method={}_plus=weekday_{}".format(method, datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")))
...
if __name__ == "__main__":
    ...
    f_patterns = [
                  ...
                  #"/Users/RungChiChen/Documents/programs/kaggle/cases/Expedia Hotel Recommendations/input/test/user_location_country/test_user_location_country=205.csv",
                  #"/Users/RungChiChen/Documents/programs/kaggle/cases/Expedia Hotel Recommendations/input/test/test_user_location_country=*.csv",
