    ...
    def run(self):
        while True:
            ...
            if self.method == self.strategy_engine.STRATEGY_MOST_POPULAR:
                ...
                metrics, (min_x, len_x), (min_y, len_y) = self.strategy_engine.get_most_popular_metrics(filepath_train, filepath_train_pkl, f, self.n_top, self.criteria[0], self.criteria[1], self.n_jobs/2)
                ...
                metrics, mapping, score = self.strategy_engine.get_kdtree(filepath_train, filepath_train_pkl, f, self.n_top, self.n_jobs/2)
            else:
